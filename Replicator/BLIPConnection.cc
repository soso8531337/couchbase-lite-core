...
#pragma mark - BLIP I/O:
    ...
    protected:
        ...
        virtual void onMessage(fleece::slice message, bool binary) override; // WebSocket message

    private:
        Retained<MessageOut> popNextMessage();
        void requeue(MessageOut*);
        void _requeue(MessageOut*);
    ...
    BLIPIO::BLIPIO(Connection *connection)
    :_connection(connection)
    { }


    void BLIPIO::enqueue(MessageOut *msg) {
        {
    ...
    void BLIPIO::_requeue(MessageOut *msg) {
        auto i = _queue.end();
    ...
    void BLIPIO::onWriteable() {
        ...
        {
            lock_guard<mutex> lock(_ioMutex);
    ...
    // Received a frame:
    void BLIPIO::onMessage(slice frame, bool binary) {
        if (!binary) {
